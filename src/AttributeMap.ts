...
export namespace AttributeMap {
	export function compose(a: AttributeMap = {}, b: AttributeMap = {}, keepNull = false): AttributeMap | undefined {
		...
		for (const key in a) {
			if (isNestedMap(a[key]) && isNestedMap(b[key])) {
				const nestedComposed = AttributeMap.compose(a[key] as AttributeMap, b[key] as AttributeMap, keepNull)
	...
	export function transform(
		...
		const attributes = Object.keys(b).reduce<AttributeMap>((attrs, key) => {
			if (isNestedMap(a[key]) && isNestedMap(b[key])) {
				const attr = AttributeMap.transform(a[key] as AttributeMap, b[key] as AttributeMap, priority)
				if (attr !== undefined) {
					attrs[key] = attr
				}
