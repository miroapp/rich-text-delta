...
describe('diff()', () => {
	...
	it('combination', () => {
		...
		const expected = new Delta().insert('Good', {bold: true}).insert('dog', {italic: true}).delete(6)
		expect(a.diff(b)).toEqual(expected)
	})
	...
	describe('nested attributes', () => {
		...
		it('diffs a nested change spanning inserts', () => {
			...
			const b = new Delta().insert('A', {comment: {'1': true}}).insert('B', {comment: {'2': true}})
			const expected = new Delta().retain(1, {comment: {'99': null}}).retain(1, {comment: {'2': true}})
			const diffed = a.diff(b)
			expect(expected).toEqual(diffed)
			expect(a.compose(diffed)).toEqual(b)
		})
