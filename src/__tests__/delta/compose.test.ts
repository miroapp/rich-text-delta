...
describe('compose()', () => {
	...
	it('nested attributes splits inserts', () => {
		...
		const b = new Delta().retain(1, {comment: {'2': true}})
		const expected = new Delta().insert('A', {comment: {'1': true, '2': true}}).insert('B', {comment: {'1': true}})
		expect(a.compose(b)).toEqual(expected)
	})

	it('nested attributes joins inserts', () => {
		const a = new Delta().insert('A', {comment: {'1': true, '2': true}}).insert('B', {comment: {'1': true}})
	...
	it('composes a nested change spanning inserts where one lacks the map', () => {
		...
		const expected = new Delta().insert('A', {comment: {'1': true, '2': true}}).insert('B', {comment: {'2': true}})
		expect(a.compose(b)).toEqual(expected)
	})
	...
	describe('custom embed handler', () => {
		...
		it('retain a number with an embed', () => {
			...
			const expected = new Delta().retain({delta: [{insert: 'b'}]}, {bold: true}).retain(9, {bold: true})
			expect(a.compose(b)).toEqual(expected)
		})
