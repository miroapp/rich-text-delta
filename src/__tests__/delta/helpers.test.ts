...
describe('helpers', () => {
	...
	describe('eachLine()', () => {
		it('expected', () => {
			...
			expect(spy).toHaveBeenNthCalledWith(1, new Delta().insert('Hello'), {}, 0)
		...
		it('trailing newline', () => {
			...
			expect(spy).toHaveBeenNthCalledWith(1, new Delta().insert('Hello'), {}, 0)
			expect(spy).toHaveBeenNthCalledWith(2, new Delta().insert('World!'), {}, 1)
		})
	...
	describe('iteration', () => {
		const delta = new Delta().insert('Hello').insert({image: true}).insert('World!')
	...
	describe('length()', () => {
		it('document', () => {
			const delta = new Delta().insert('AB', {bold: true}).insert({embed: 1})
			expect(delta.length()).toEqual(3)
		})

		it('mixed', () => {
			const delta = new Delta().insert('AB', {bold: true}).insert({embed: 1}).retain(2, {bold: null}).delete(1)
