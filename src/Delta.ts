...
const getEmbedTypeAndData = (a: Op['insert'] | Op['retain'], b: Op['insert']): [string, unknown, unknown] => {
		...
		while (thisIter.hasNext() || otherIter.hasNext()) {
			if (otherIter.peekType() === 'insert') {
				...
				if (otherOp.retain) {
					...
					(typeof thisOp.retain === 'number' || (typeof thisOp.retain === 'object' && thisOp.retain !== null))
				) {
					delta.push(otherOp)
				}
			}
